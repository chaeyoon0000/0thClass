...
function getAllMessengers(str) {
	...
	$.ajax({
		type: "get",
		url: "/mybatis-spring-mvc/messenger/getMessengerAll.do/" + str,
		processData: false,
		success: function(responseJson){	// object parsed from JSON text	
			...
			if (responseJson.messengers.length > 0) {			
				...
				for(var i = 0; i < responseJson.messengers.length; i++) {
					var messenger = responseJson.messengers[i]; 
									
					var url = "/mybatis-spring-mvc/messenger/messengerDetail.do/" + messenger.mNum;
					var line = "<tr id='tr'><th>" + (i+1) + "</th>"
								+ "<td><a href=" + "'" + url + "'>" + messenger.title + "</a></td>"
								+ "<td>" + messenger.sender.id + "</td>"
								+ "<td>" + messenger.receiver.id + "</td>"
								+ "<td>" + messenger.date + "</td></tr>";
...
// 	            });
			...
			//	$("#result3").append(document.createElement("tbody")); 
				...
				for(var i = 0; i < responseJson.messengers.length; i++) {
					//var num = i;
					var messenger = responseJson.messengers[i]; 
// 					$("#result3 tbody").append("<tr><th>" + (i+1) + "</th>"); 

// 	살려				$("#result3").append("<tr><th>" + (i+1) + "</th>");
					//$("#result3 tbody").append("<tr><td>" + (i+1) + "</td>");
// 					$("#result3 > td:last").append("<td>");
// 					var url = "/mybatis-spring-mvc/messenger/messengerDetail.do/" + messenger.mNum;
// 					$("#result3 > tbody:last").attr("href", url);
	
// 여기여기3 삭제하면 여기부터 살려					var td = document.createElement("td"); alert("1");
// 					var a = document.createElement("a"); /* alert("2"); */
// 					var url = "/mybatis-spring-mvc/messenger/messengerDetail.do/" + messenger.mNum; /* alert("3"); */
// 					a.setAttribute('href', url); /* alert("4"); */
// 					a.innerHTML = messenger.title;/*  alert("5"); */
// 					td.appendChild(a);  alert("6"); 
// 여기여기3 삭제하면 여기까지 살려	

					//$("#result3 tbody").append("<tr><th>" + (i+1) + "</th>");
// 					$("#result3 tbody").append(td);  alert("dudd");//$("#result3 tbody").apeend(td); alert("dudd");
// 					$("#result3 tbody").append("<td>" + messenger.receiver.id + "</td>");
// 					$("#result3 tbody").append("<td>" + messenger.date + "</td></tr>"); alert("dud");

// 	살려							$("#result3").append(td);  alert("dudd");//$("#result3 tbody").apeend(td); alert("dudd");
// 	살려							$("#result3").append("<td>" + messenger.receiver.id + "</td>");
// 	살려							$("#result3").append("<td>" + messenger.date + "</td></tr>"); alert("dud");
					
					
// /* 여기여기 */			var index = "th" + (i+1);/* alert(index); */
// 					$("#result3").append("<tr><th id='" + index + "'>" + (i+1) + "</th>"
// 										+ "<td>" + messenger.receiver.id + "</td>"
// 										+ "<td>" + messenger.date + "</td></tr>");   /* 여기여기 */	

// 					$("#result3").append(td);		alert("DDD");			

// /* 여기여기2 */			var index = "th" + (i+1);/* alert(index); */
// 					$("#result3 tbody").append("<tr><th id='" + index + "'>" + (i+1) + "</th>"
// 										+ "<td>" + messenger.receiver.id + "</td>"
// 										+ "<td>" + messenger.date + "</td></tr>");   /* 여기여기2 */
// 					var id = '"#' + index + '"'; alert(id);
// 					$(id).append(td);		alert(id);	
...
/* 여기여기3 */					
			 	...
			 	$('#tr').remove();
			 	$("#result3").append(document.createElement("tbody"));
			 	$('#result3 > tbody:last').append("<center>No result!</center>");
			}
// 			else {
...
<!-- 					<li class="nav-item"><a class="nav-link active" href="/mybatis-spring-mvc/messenger/listMessengers.do/"> -->
<%-- 						${checkStatusList[0]}</a></li> --%>
					<li class="nav-item"><label class="btn btn-primary text-white">
						<input type="radio" id="uNum" name="uNum" value="0"  onClick="getAllMessengers(${sessionScope.user.uNum});">
<!-- 						<a class="nav-link active" href="/mybatis-spring-mvc/messenger/listMessengers.do/"> -->
						${checkStatusList[0]}</label></li><!-- </a></li> -->
					<li class="nav-item"><label class="btn btn-primary text-white">
						<input type="radio" id="uNum" name="uNum" value="1"  onClick="getReceivedMessengers(${sessionScope.user.uNum});">
							${checkStatusList[1]}</label></li>
					<li class="nav-item"><label class="btn btn-primary text-white">
						<input type="radio" id="uNum" name="uNum" value="2"  onClick="getSendMessengers(${sessionScope.user.uNum});">
